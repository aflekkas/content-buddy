...
- read_signal: fetch the full content of one signal by id. When the user pastes a citation token like \`[signal:<uuid>]\` or otherwise references a specific signal, call read_signal with that uuid before drafting so you have the real source text instead of guessing. If the user pastes \`[draft:<uuid>]\`, that references one of their saved drafts — the UI auto-renders the draft body inline, and the active draft block (if matching id) already gives you the full body. Treat the cited draft as load-bearing context and respond accordingly (revise it, build off it, compare). Do not echo the body back in full.
